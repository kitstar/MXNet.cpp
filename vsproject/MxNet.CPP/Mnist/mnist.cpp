...
/* virtual */ Symbol Mnist::BuildNetwork()
{
    ...
    auto pool1 = Pooling("Convolution1 pooling", tanh1, Shape(2, 2), PoolingPoolType::max, false, Shape(2, 2));
    ...
    auto pool2 = Pooling("Convolution2 pooling", tanh2, Shape(2, 2), PoolingPoolType::max, false, Shape(2, 2));
...
/* virtual */ void Mnist::Predict(KVStore *kv_store, bool is_validation)
{
    ...
    // Start Prediction    
    auto data_reader = GetFileReader(input_image_data, batch_size_, 28 * 28, kv_store->GetRank(), kv_store->GetNumWorkers(), sizeof(uint32_t) * 4);
    DataReader *label_reader = nullptr;
    if (is_validation)
        label_reader = GetFileReader(input_label_data, batch_size_, 1, kv_store->GetRank(), kv_store->GetNumWorkers(), sizeof(uint32_t) * 2);
